...
FILE *tune_out = stdout;
FILE *tune_err = stderr;
...
tune_list_mul_n ()
{
  ...
  for (n = 1; n < TUNE_LIST_MUL_N_MAX_SIZE; n++)
    {
      if (tune_verbose)
        printf ("%"PRIu64":", (long long unsigned int)n);
      __k = 1;
...
int tune_main(int v, int min, int max, unsigned long int seed, const char *outfname, const char *errfname)
{
  spv_size_t i;

  if (errfname != NULL) {
      tune_err = fopen(errfname, "w");
  }
  if (tune_err == NULL) {
      return -1;
  }
  if (outfname != NULL) {
      tune_out = fopen(outfname, "w");
  }
  if (tune_out == NULL) {
      fprintf(tune_err, "could not open '%s'\n", outfname);
      fclose(tune_err);
      return -1;
  }
  tune_verbose = v;
  if (min != -1) min_log2_len = min;
  if (max != -1) max_log2_len = max;
  ...
  fprintf (tune_out, "#define NTT_GFP_TWIDDLE_DIF_BREAKOVER %lu\n",
  ...
  fprintf (tune_out, "#define POLYINVERT_NTT_THRESHOLD %lu\n", 
  ...
  fprintf (tune_out, "#define POLYEVALT_NTT_THRESHOLD %lu\n", 
  ...
  fprintf (tune_out, "#define MPZSPV_NORMALISE_STRIDE %lu\n", 
