...
/* GMP uses the opposite convention for the precomputed inverse in redc_n
  ...
  if (verbose)
    {
      fprintf (err, "******************\n");
      fprintf (err, "Time in microseconds per call, size=%lu (iter=%lu):\n",
               N, iter);

      /* basic operations */
      fprintf (err, "mpn_mul_n  = %.3f\n",
               (double) tmul * 1e3 / (double) iter);
      fprintf (err, "mpn_sqr    = %.3f\n",
               (double) tsqr * 1e3 / (double) iter);
#ifdef HAVE___GMPN_REDC_1
      fprintf (err, "mpn_redc_1 = %.3f",
    ...
    fprintf (err, "mpn_redc_n = disabled\n");
#endif
  ...
  /* modular multiplication */
    ...
    fprintf (err, "mul+redc_n = disabled\n");
#endif
  ...
  /* modular squaring */
    ...
    fprintf (err, "sqr+redc_n = disabled\n");
#endif

#ifdef HAVE_NATIVE_MULREDC1_N
      /* multiplication of n limbs by one limb */
      fprintf (err, "mulredc1   = %.3f\n",
...
int bench_main(int v, int min, int max, const char *outfname, const char *errfname)
{
  int i;
  int minsize = 1;
  int maxsize = MAXSIZE;
  ...
  verbose = v;
  if (min != -1) minsize = min;
  if (max != -1) maxsize = max;
  for (i = minsize; i <= maxsize; ++i) {
      bench(i);
  }
  fprintf (out, "/* 0:mulredc 1:mul+redc_1 2:mul+redc_2 3:mul+redc_n */\n");
  fprintf (out, "#define TUNE_MULREDC_TABLE {0");
  for (i = 1; i <= maxsize; i++) {
      fprintf (out, ",%d", tune_mul[i]);
  }
  fprintf (out, "}\n");
  fprintf (out, "/* 0:mulredc 1:sqr+redc_1 2:sqr+redc_2 3:sqr+redc_n */\n");
  fprintf (out, "#define TUNE_SQRREDC_TABLE {0");
  for (i = 1; i <= maxsize; i++) {
      fprintf (out, ",%d", tune_sqr[i]);
  }
  fprintf (out, "}\n");
  fflush (out);
